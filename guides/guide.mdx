...
```bash cURL
curl -X POST https://api.xerpio.se/public/v1/invoices \
...
const response = await fetch('https://api.xerpio.se/public/v1/invoices', {
  method: 'POST',
  headers: {
    'x-api-key': 'YOUR_API_KEY',
...
response = requests.post(
    'https://api.xerpio.se/public/v1/invoices',
    headers={
        'x-api-key': 'YOUR_API_KEY',
        'Content-Type': 'application/json'
    },
...
data = response.json()
print(f"Invoice ID: {data['invoiceId']}")
```
...
```bash cURL
curl -X GET https://api.xerpio.se/public/v1/invoices/inv_abc123def456/status \
  -H "x-api-key: YOUR_API_KEY"
```

```javascript JavaScript
const response = await fetch(
  'https://api.xerpio.se/public/v1/invoices/inv_abc123def456/status',
  {
    headers: {
      'x-api-key': 'YOUR_API_KEY'
    }
...
Once processing is complete, retrieve the full invoice data:

<CodeGroup>

```bash cURL
curl -X GET https://api.xerpio.se/public/v1/invoices/inv_abc123def456 \
  -H "x-api-key: YOUR_API_KEY"
```

```javascript JavaScript
const response = await fetch(
  'https://api.xerpio.se/public/v1/invoices/inv_abc123def456',
  {
    headers: {
      'x-api-key': 'YOUR_API_KEY'
    }
...
Update invoice metadata using the PATCH endpoint:

<CodeGroup>

```bash cURL
curl -X PATCH https://api.xerpio.se/public/v1/invoices/inv_abc123def456 \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "externalId": "updated_inv_12345",
...
```bash cURL
curl -X GET "https://api.xerpio.se/public/v1/invoices?limit=20&page=1&status=SUCCESS" \
  -H "x-api-key: YOUR_API_KEY"
```
...
const response = await fetch(
  `https://api.xerpio.se/public/v1/invoices?${params}`,
  {
    headers: {
      'x-api-key': 'YOUR_API_KEY'
    }
...
- Store invoice IDs for later retrieval
...
class InvoiceProcessor {
  constructor(apiKey) {
    this.apiKey = apiKey;
    this.baseUrl = 'https://api.xerpio.se';
  }
  ...
  async getInvoice(invoiceId) {
    const response = await fetch(
      `${this.baseUrl}/public/v1/invoices/${invoiceId}`,
      {
        headers: {
          'x-api-key': this.apiKey
        }
  ...
  async waitForCompletion(invoiceId, maxWaitTime = 300000) {
...
try {
  ...
  const invoice = await processor.waitForCompletion(result.invoiceId);
...
- Status Page: https://status.xerpio.se
